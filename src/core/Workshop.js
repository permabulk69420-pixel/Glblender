...
export class Workshop {
  constructor(container) {
    ...
    this.environment = pmrem.fromScene(room, .04).texture; this.scene.environment = this.environment; pmrem.dispose(); room.dispose();
    this.ambient = new THREE.HemisphereLight('#e9f0fb', '#666866', 2.1); this.scene.add(this.ambient);
    const key = new THREE.DirectionalLight('#fff6e8', 3.2); key.position.set(2,5,3); this.scene.add(key);
    const rim = new THREE.DirectionalLight('#c3ddf3', 2.1); rim.position.set(-4,3,-4); this.scene.add(rim);
    this.floor = new THREE.Mesh(new THREE.PlaneGeometry(200,200), new THREE.MeshStandardMaterial({color:'#232b35',roughness:.98,metalness:.08}));
    ...
    const plinth = new THREE.Mesh(new THREE.CylinderGeometry(1.14,1.1,.07,80),new THREE.MeshStandardMaterial({color:'#303b45',roughness:.7,metalness:.55})); plinth.position.y=.79; this.table.add(plinth);
    ...
    const surface = new THREE.GridHelper(1.9,19,'#52636a','#3e4c54'); surface.position.y=.829; surface.material.transparent=true; surface.material.opacity=.48; this.table.add(surface);
